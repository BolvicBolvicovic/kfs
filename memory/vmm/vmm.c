...
void	vmm_temp_unmap(uint32_t virtual_addr, uint32_t nb_blocks);
void*	vmm_temp_map(uint32_t physical_addr, uint32_t nb_blocks);

static void
vmm_map_page(pd_entry* dir, uint32_t phys, uint32_t virt, uint32_t flags)
{

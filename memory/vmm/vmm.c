...
p_dir*
vmm_get_dir() { return _current_dir; }

void
vmm_map_page(void* phys, void* virt)
{
...
uint32_t
vmm_find_next_free_s(size_t nb_blocks)
{
...
void*
vmm_alloc_blocks(size_t nb_blocks)
{
	...
	{
        vmm_alloc_page(&page_table[pt_index + i]);
    }
...
vmm_free_blocks(uint32_t virtual_addr, uint32_t nb_blocks)
{
    ...
    for (size_t i = 0; i < nb_blocks; i++)
	{
        vmm_free_page(&page_table[pt_index + i]);
    }
}

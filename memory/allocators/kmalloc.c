#include <compiler.h>
#include <memory/vmm/vmm.h>
#include <processes/locks/spinlock.h>

#define MAX_ALLOC_C_SAME_TIME		PAGE_SIZE
#define MAX_ALLOC_B_SAME_TIME		PAGE_SIZE / 8
#define MAX_SIZE_ALLOC_SAME_TIME	0xFFFFF
#define MAX_SIZE_B_ALLOC		2048
#define MIN_SIZE_B_ALLOC		8
#define MAX_BITMAP_ELEM			16
#define FREE_LIMIT_B_ALLOC		0xF00

#define CHECK_UPPER_HALF_BITMAP		0xFFFF0000
#define CHECK_REMAINING_HALF_BITMAP	0xFF00FF00
#define CHECK_NEXT_QUARTER_BITMAP	0xF0F0F0F0
#define CHECK_NEXT_EIGHTH_BITMAP	0xCCCCCCCC
#define CHECK_FINAL_PART_BITMAP		0xAAAAAAAA
#define CHECK_LAST_BIT_BITMAP		0x80000000

#define PAGE_ADDR_MASK			0xFFFFF000
#define OFFSET_ADDR_MASK		~PAGE_ADDR_MASK

void*	kmalloc(u32 size);

typedef struct
{
    u32	virt_addr;
    u16	nb_blocks;
    u16	free;
} __packed cont_map_t;

typedef struct
{
    u32 virt_addr;
    u16 size_type : 12;
    u16 offset_next_free : 10;
    u8  bitmap_offset: 4;
    u8  bitmap_elem_offset: 6;
    u32 bitmap[MAX_BITMAP_ELEM];
} __packed bini_map_t;

static cont_map_t	continuous_allocator_map[MAX_ALLOC_C_SAME_TIME]	= {0};
static bini_map_t	bining_allocator_map[MAX_ALLOC_B_SAME_TIME]	= {0};

static SPINLOCK_DEFINE(sl_continuous_allocator);
static SPINLOCK_DEFINE(sl_bining_allocator);

static void*
bining_allocator(u32 size)
{
	u32	i;
	u32	size_type;

	for (size_type = 8; size_type < size; size_type <<= 1);

	spinlock_lock(&sl_bining_allocator);

	for (i = 0; i < MAX_ALLOC_B_SAME_TIME; i++)
	{
		if (!bining_allocator_map[i].virt_addr) break;
		if (bining_allocator_map[i].size_type != size_type) continue;
		if (bining_allocator_map[i].offset_next_free * bining_allocator_map[i].size_type
			== PAGE_SIZE) continue;
		
		u32	virt_addr = bining_allocator_map[i].virt_addr
			+ bining_allocator_map[i].offset_next_free
			* bining_allocator_map[i].size_type;

		bining_allocator_map[i].offset_next_free++;
		bining_allocator_map[i].bitmap[bining_allocator_map[i].bitmap_offset]
			|= (1 << bining_allocator_map[i].bitmap_elem_offset);

		if (++bining_allocator_map[i].bitmap_elem_offset == 32)
		{
			bining_allocator_map[i].bitmap_elem_offset = 0;
			bining_allocator_map[i].bitmap_offset++;
		}

		spinlock_unlock(&sl_bining_allocator);

		return (void*)virt_addr;
	}

	if (i == MAX_ALLOC_B_SAME_TIME) return kmalloc(MAX_SIZE_B_ALLOC + 1);

	for (i = 0; i < MAX_ALLOC_B_SAME_TIME; i++)
	{
		if (!bining_allocator_map[i].size_type) break;
	}

	void*	new_page = vmm_alloc_kblocks(1);

	if (new_page == 0)
	{
		spinlock_unlock(&sl_bining_allocator);

		return 0;
	}

	bining_allocator_map[i].virt_addr		= (u32)new_page;
	bining_allocator_map[i].size_type		= size_type;
	bining_allocator_map[i].offset_next_free	= 1;
	bining_allocator_map[i].bitmap[0]		|= 1;
	bining_allocator_map[i].bitmap_elem_offset	= 1;
	
	spinlock_unlock(&sl_bining_allocator);

	return new_page;
}

s32
bining_allocator_free(u32 addr)
{
	u32	page	= addr & PAGE_ADDR_MASK;
	u32	i	= 0;

	for (; i < MAX_ALLOC_B_SAME_TIME; i++)
	{
		if (!bining_allocator_map[i].virt_addr) break;
		if (bining_allocator_map[i].virt_addr == page)
		{
			u16	bitmap_index		= (addr & OFFSET_ADDR_MASK)
				/ bining_allocator_map[i].size_type / 32;
			u8	bitmap_elem_offset	= (addr & OFFSET_ADDR_MASK)
				/ bining_allocator_map[i].size_type % 32;

			bining_allocator_map[i].bitmap[bitmap_index] &= ~(1 << bitmap_elem_offset);

			if (bining_allocator_map[i].offset_next_free * bining_allocator_map[i].size_type
				>= FREE_LIMIT_B_ALLOC)
			{
				u16	count = 0;

				for (u32 j = 0; j < MAX_BITMAP_ELEM; j++)
				{
					if (!bining_allocator_map[i].bitmap[j]) count++;
				}

				if (count == MAX_BITMAP_ELEM)
				{
					vmm_free_blocks(bining_allocator_map[i].virt_addr, 1);
					// Do not set virt_addr to 0 as it is used to find the edge
					// of the bining_allocator_map
					bining_allocator_map[i].size_type = 0;
					bining_allocator_map[i].offset_next_free = 0;
					memset(bining_allocator_map[i].bitmap, 0, MAX_BITMAP_ELEM);
					bining_allocator_map[i].bitmap_elem_offset = 0;
				}
			}
			
			return 1;
		}
	}
	
	return 0;
}

void*
kmalloc(u32 size)
{
	if (size <= MAX_SIZE_B_ALLOC) return bining_allocator(size);
	
	u32	total_pages_needed = size / PAGE_SIZE + (size % PAGE_SIZE ? 1 : 0);
	u32	i;
	u32	j;

	spinlock_lock(&sl_continuous_allocator);

	for (j = 0, i = 0; i < MAX_ALLOC_C_SAME_TIME; i++)
	{
		if (!continuous_allocator_map[i].free)
		{
			// Note: In case the block has not been initialized (meaning free), break
			if (continuous_allocator_map[i].virt_addr == 0) break;
			j++;
	        }
		else if (continuous_allocator_map[i].nb_blocks == total_pages_needed)
		{
			continuous_allocator_map[i].free = 0;
			vmm_set_flags_pages(
				continuous_allocator_map[i].virt_addr,
				continuous_allocator_map[i].nb_blocks, PE_WRITABLE, 1);

			spinlock_unlock(&sl_continuous_allocator);

			return (void*)continuous_allocator_map[i].virt_addr;
	        }
	}

	if (i == MAX_ALLOC_C_SAME_TIME && j == MAX_ALLOC_C_SAME_TIME)
	{
		spinlock_unlock(&sl_continuous_allocator);

		return 0;
	}

	if (i == MAX_ALLOC_C_SAME_TIME)
	{
		for (i = 0; i < MAX_ALLOC_C_SAME_TIME; i++)
		{
			// Note: In case the block has not been initialized (meaning free), break
			if (i == j) break;
	
			if (continuous_allocator_map[i].free)
			{
				vmm_free_blocks(continuous_allocator_map[i].virt_addr,
					continuous_allocator_map[i].nb_blocks);
				
				break;
			}
		}
	}

	void*	block_virt_addr = vmm_alloc_kblocks(total_pages_needed);

	if (block_virt_addr == 0)
	{
		spinlock_unlock(&sl_continuous_allocator);

		return 0;
	}

	continuous_allocator_map[i].virt_addr	= (u32)block_virt_addr;
	continuous_allocator_map[i].nb_blocks	= total_pages_needed;
	continuous_allocator_map[i].free	= 0;
	
	spinlock_unlock(&sl_continuous_allocator);
		
	return block_virt_addr;
}

void
kfree(void* virt_addr)
{
	if (virt_addr == 0) goto error;
	if (bining_allocator_free((u32)virt_addr)) return;

	for (u32 i = 0; i < MAX_ALLOC_C_SAME_TIME && continuous_allocator_map[i].virt_addr; i++)
	{
		if (continuous_allocator_map[i].virt_addr == (u32)virt_addr)
		{
			vmm_set_flags_pages(continuous_allocator_map[i].virt_addr,
				continuous_allocator_map[i].nb_blocks, PE_WRITABLE, 0);
			continuous_allocator_map[i].free = 1;

			return;
		}
	}

error:	printf("ERROR: Invalid free\n");
}

u32
kget_size(void* virt_addr)
{
	if (virt_addr == 0) return 0;

	u32	i;
	u32	page = (u32)virt_addr & PAGE_ADDR_MASK;

	for (i = 0; i < MAX_ALLOC_B_SAME_TIME; i++)
	{
		if (!bining_allocator_map[i].virt_addr) break; 

		if (bining_allocator_map[i].virt_addr == page)
			return bining_allocator_map[i].size_type;
	}

	for (i = 0; i < MAX_ALLOC_C_SAME_TIME; i++)
	{
		if (!continuous_allocator_map[i].free
			&& !continuous_allocator_map[i].nb_blocks
			&& !continuous_allocator_map[i].virt_addr) break;

		if (continuous_allocator_map[i].virt_addr == (u32)virt_addr)
			return continuous_allocator_map[i].nb_blocks * PAGE_SIZE;
	}

	return 0;
}
